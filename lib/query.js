 ...
 */
async function handleCancellations(repo, artifactClient) {
    let isCancelled = false;
    process.on("SIGINT", () => {
        isCancelled = true;
        console.log(`SIGINT received, uploading error artifact for repo ${repo.nwo}`);
    });
    process.on("SIGTERM", () => {
        isCancelled = true;
        console.log(`SIGTERM received, uploading error artifact for repo ${repo.nwo}`);
    });
    if (isCancelled) {
